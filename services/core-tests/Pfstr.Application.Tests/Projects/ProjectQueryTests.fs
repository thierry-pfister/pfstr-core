...
let private makeStub (projects: Project list) =
    { new IProjectRepository with
        member _.FindById id    = async { return projects |> List.tryFind (fun p -> p.Id = id) }
        member _.FindBySlug s   = async { return projects |> List.tryFind (fun p -> p.Slug = s) }
        member _.FindAll()      = async { return projects }
        member _.Save _         = async { return () } }

[<Fact>]
let ``GetProject returns project for existing slug`` () =
    let project = makeProject "Test" "test-project"
    let repo = makeStub [project]
    match GetProject.handle repo { Slug = "test-project" } |> Async.RunSynchronously with
...
let ``ListProjects returns all projects when no status filter`` () =
    ...
    let repo = makeStub [draft; published]
    let result = ListProjects.handle repo { Status = None } |> Async.RunSynchronously
...
let ``ListProjects filters by Published status`` () =
    ...
    let published = makeProject "Published" "published-project" |> Project.publish now |> function Ok p -> p | Error e -> failwith e
    let repo = makeStub [draft; published]
    let result = ListProjects.handle repo { Status = Some Published } |> Async.RunSynchronously
